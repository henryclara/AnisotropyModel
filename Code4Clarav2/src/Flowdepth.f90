...
!------------------------------------------------------------------------------
  ...
  IF ( (.NOT. AllocationsDone) .OR. Solver % Mesh % Changed  ) THEN
     ...
     CALL INFO( SolverName, 'Memory allocation done.',Level=4 )
  END IF
  ...
  ELSE
     IF (CalcFree) THEN
               CALL INFO(SolverName, 'Free surface variable will be calculated', Level=4)
...
!looks like Depth/Thic gradient, but is for some reason Bed and Surf gradient
